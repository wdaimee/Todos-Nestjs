...
export class TodoService {
    ...
    findAllTodos(user: User): Promise<Todo[]> {
        return this.todoRepository.find({ where: { user }, order: { dueDate: "ASC" } });
    }

    // Find all Todos for today and future
    findAllTodosToday(user: User): Promise<Todo[]> {
        return this.todoRepository.find({ where: { user, dueDate: MoreThanOrEqual(new Date()) }, order: { dueDate: "ASC" } });
    }
